...
It was nice to get a glimpse of React "under the hood", in the sense that I finally saw with my own eyes how React separates the definition of its core pieces (its elements), which is done via the React API; from their rendering to whatever UI system we choose. In the case of web, this is done via the ReactDOM API, but I'm guessing there are similar APIs to render the same elements to an Android UI or an iOS UI.

Really appreciated the link to that "Declarative vs. Imperative" article. The shortest and, at the same time, most complete I've read on the matter so far.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=jaime.ferbec%40gmail.com).
