...
I learned how to read and set values for form elements, which means I now know how to read and "write" (i.e. render) data from and to a React built web app.

I additionally learnt about:
* `console.dir`. How could I have not used that console method before?
* HTML properties are set by prepending them with an `html`. For instance: `htmlFor`.
* Events fired in a React web app are actually not native Events, but Synthetic React Events.

I was surprised by the behaviour of `onChange` in the extra credit 2 exercise, as it fires when I would expect an `onInput` event to fire. `onChange` should only fire when user's focus changes to another element, if I'm not mistaken.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=jaime.ferbec%40gmail.com).
