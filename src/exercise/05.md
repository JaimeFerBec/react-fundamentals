...
It was very effective to be reminded that, even though the syntax we are writing is similar to HTML, we are actually managing the DOM API more directly. Therefore, we should use `className` instead of `class`, and `style` is an `Object`, and not a `String`.

I also liked to be forced to create a React Component on the extra credit exercises to make my brain remember about the previous lessons.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=jaime.ferbec%40gmail.com).
