...
JSX provides us with a way to write declarative React to render a UI, which is very close to writing HTML, with a few caveats and also advantages over it (via Javascript interpolation). Babel compiles it to React API code, which can in turn be rendered by any React UI engine (such as ReactDOM for instance, if we want the result to be rendered on a web browser). This is very powerful, because if we already knew how to build user interfaces for web, by just getting a bit more of knowledge, we can now write user interfaces for any UI system React renders to.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=jaime.ferbec%40gmail.com).
